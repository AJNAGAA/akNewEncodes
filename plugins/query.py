...
async def Cb_Handle(bot: Client, query: CallbackQuery):
    ...
    if data.startswith('stats'):
        ...
        try:
            ...
            ffmpeg = "-preset ultrafast -c:v libx265 -s 640x360 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset ultrafast -c:v libx265 -s 854x480 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
            await CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb)
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset ultrafast -c:v libx265 -s 960x540 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
            await CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb)
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset ultrafast -c:v libx265 -s 1280x720 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
            await CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb)
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset ultrafast -c:v libx265 -s 1920x1080 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
            await CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb)
        ...
        try:
            c_thumb = await db.get_thumbnail(query.from_user.id)
            ffmpeg = "-preset ultrafast -c:v libx265 -s 3840x2160 -x265-params 'bframes=10:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf 30 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1"
            await CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb)
